...
(function(scene, projector, mouseVector, raycaster, wiggle, camera, renderer) {;
    ...
    function sphere_mousedown(e) {
        ...
        intersects = raycaster.intersectObjects( currentGroup.children );
    }
    ...
    mouseVector.unproject( camera);
    raycaster.set( camera.position, mouseVector.sub( camera.position ).normalize() );
    //var intersects = raycaster.intersectObjects( currentGroup.children );
    ...
    function animate() {
        ...
        if(wiggle) {
            for(var i in currentGroup.children) {
                var sphereObject = currentGroup.children[i];
    ...
    jQuery("#clickObject").on("keydown", function(e) {
        if(e.keyCode === 13) {
            ...
            try {
                ...
                y = parseInt(parts[1]);
                z = parseInt(parts[2]);
                activate(currentGroup, x,y,z);
            } catch(ex){}

            clickObject.val("");
        }
    });
    jQuery("#regen").click(theMagic);
})(scene, projector, mouseVector, raycaster, wiggle, camera, renderer);

function activate(_group, x,y,z) {
    for(var i in _group.children) {
        var sphereObject = _group.children[i];
